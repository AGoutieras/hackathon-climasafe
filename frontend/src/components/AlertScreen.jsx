...
export function AlertScreen() {
  ...
  const apparent =
    typeof riskData?.apparent_temperature === "number"
  ...
  const forecastPeak =
    apparent != null
      ? Math.max(
          apparent,
          (tempRaw != null ? Math.round(tempRaw) : apparent) + 1,
        )
      : null;
  ...
  const warningLabel =
    riskData?.score != null
      ? `Indice ${riskData.score}/100`
      : ui.summaryZoneLabel;
  const summaryCardCls = hasActive
  ...
  const timelineAlerts = [...displayAlerts].sort(
    ...
    >
      ...
      <div className="pt-4 pb-6">
        <div className="flex items-center gap-3 mb-2">
          <div
            className={`w-12 h-12 ${ui.panelAccentBg} rounded-full flex items-center justify-center`}
          >
            <Bell className="text-white" size={24} />
          </div>
          <div>
            <h1 className="text-2xl sm:text-4xl text-slate-900">
              Sois préparé ⚠️
            </h1>
            <p className="text-slate-600">
              {activeAlerts.length} alerte{activeAlerts.length !== 1 ? "s" : ""}{" "}
              active{activeAlerts.length !== 1 ? "s" : ""}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className={`p-4 sm:p-6 mb-6 rounded-xl shadow-xl ${summaryCardCls}`}>
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1">
            <p className={`text-sm ${summarySubCls} mb-1`}>
              {hasActive ? "Alerte en cours" : "Statut actuel"}
            </p>
            <h2 className="text-xl sm:text-2xl mb-2">
              {hasActive ? ui.summaryTitle : "Aucune alerte en cours"}
            </h2>
            <p className={`${summarySubCls} text-sm`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-2xl sm:text-3xl">
                {tempRaw != null ? `${Math.round(tempRaw)}°C` : "--"}
              </p>
            </div>
            <div>
              <p className={`text-sm ${summarySubCls} mb-1`}>Pic attendu</p>
              <p className="text-2xl sm:text-3xl">
                {forecastPeak != null ? `${forecastPeak}°C` : "--"}
              </p>
            </div>
          </div>
        </div>
      ...
      <Card className="p-5 mb-6 shadow-sm border-slate-200">
        ...
        <ul className="space-y-3">
          {actions.map((action) => (
            <li
              key={action.number}
              className={`flex items-start gap-3 p-3 ${actionColors.bg} rounded-xl`}
            >
              <div
                className={`w-8 h-8 ${actionColors.icon} rounded-full flex items-center justify-center flex-shrink-0 mt-0.5`}
              >
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1">
              Vue synthétique des alertes locales et de leur état actuel.
            </p>
          </div>
          <div className="shrink-0 rounded-full bg-slate-100 px-3 py-1 text-xs text-slate-600">
            {timelineAlerts.length} événement
            {timelineAlerts.length !== 1 ? "s" : ""}
          </div>
        </div>
        ...
        <div className="space-y-3">
          {timelineAlerts.map((alert, index) => {
            ...
            const status = alert.isActive
              ...
              : index === 0
                ? alertStateConfig.monitoring
                : alertStateConfig.resolved;
            ...
            return (
              <div key={alert.id} className="flex gap-3">
                <div className="flex flex-col items-center pt-1">
                  <div
                    className={`w-3 h-3 rounded-full ${isResolved ? "bg-slate-900" : cfg.iconBg} ${alert.isActive ? "animate-pulse" : ""}`}
                  />
                  {index < timelineAlerts.length - 1 && (
                    <div className="w-px flex-1 min-h-20 bg-slate-200 mt-2" />
                  )}
                </div>
                ...
                <Card className={`flex-1 p-4 border ${cardStateClass}`}>
                  <div className="flex items-start gap-3">
                    <div
                      className={`w-12 h-12 ${isResolved ? "bg-slate-900" : cfg.iconBg} rounded-xl flex items-center justify-center text-white flex-shrink-0`}
                    >
                      <Icon size={20} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="min-w-0">
                          ...
                          <p
                            className={`text-xs mt-1 uppercase tracking-wide ${isResolved ? "text-slate-700 font-semibold" : "text-slate-500"}`}
                          >
                            {isResolved
                              ? "Alertes clôturées"
                              : alert.type === "high"
                        ...
                        <span
                          className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-medium ${status.pill}`}
                        >
                          {status.label}
                        </span>
                      </div>
                      ...
                      <p
                        className={`text-sm mb-3 ${isResolved ? "text-slate-800" : "text-slate-700"}`}
                      >
                        {alert.message}
                      </p>

                      <div
                        className={`flex flex-wrap items-center gap-3 text-xs ${isResolved ? "text-slate-700" : "text-slate-500"}`}
                      >
                        ...
                        <span>
                          {alert.isActive
                            ...
                            : isResolved
                              ? "Alertes terminées et archivées"
                              : "À garder en mémoire"}
                        </span>
                      </div>
                    </div>
      ...
      <Card className="p-5 mb-6 bg-slate-900 text-white shadow-lg">
        ...
        <p className="text-slate-300 text-sm mb-4">
          Si vous ressentez des malaises, vertiges, nausées ou maux de tête
          intenses
        </p>
        <Button
          onClick={() => {
