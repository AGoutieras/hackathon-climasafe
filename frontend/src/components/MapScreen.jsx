...
const coolColor = (score, type) => {
  if (type === "water")
    return score >= 90
      ? COOL_ZONE_COLORS.water.high
      : COOL_ZONE_COLORS.water.base;
  return COOL_ZONE_COLORS[type] ?? COOL_ZONE_COLORS.fallback;
};
const heatColor = (risk) =>
  ({ very_high: "#ef4444", high: "#f97316", medium: "#facc15" })[risk] ??
  "#fde68a";
const riskLabel = (risk) =>
  ({ very_high: "Très élevé", high: "Élevé", medium: "Modéré" })[risk] ??
  "Faible";
const spotLabel = (type) =>
  ({ park: "Zone boisée", water: "Plan d'eau", green: "Zone arborée" })[type] ??
  "Zone fraîche";
    ...
    >
      <div
        style={{
          ...
          boxShadow: selected
            ? `0 0 0 3px ${c}80,0 4px 12px rgba(0,0,0,.4)`
    ...
    >
      <div
        style={{
          ...
          boxShadow: selected
            ? "0 0 0 3px #06b6d480,0 4px 12px rgba(0,0,0,.3)"
            : "0 2px 6px rgba(0,0,0,.3)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
            ...
            <div key={i} className="flex items-start gap-3 px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-slate-800 leading-snug">
                  {s.instruction || "Continuez tout droit"}
                </p>
                {s.distance > 5 && (
    ...
    <Card className="p-5 shadow-xl border-2 mb-4" style={{ borderColor: c }}>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        {[
          ...
          {
            icon: <Clock size={12} />,
            val: `${spot.walkTime} min`,
            label: "À pied",
          },
          {
            icon: <ThermometerSnowflake size={12} />,
            val: `${spot.estimatedTemperature}°C`,
            label: "Estimé",
            accent: c,
          },
        ].map((item, i) => (
          ...
          >
            <div
              ...
              style={item.accent ? { color: c } : { color: "#94a3b8" }}
            >
              {item.icon}
            </div>
            <p
              className="text-base font-semibold text-slate-900"
              style={item.accent ? { color: c } : {}}
            >
              {item.val}
            </p>
      ...
      <div className="mb-4">
        ...
        <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${spot.coolnessScore}%`, background: c }}
          />
        </div>
      </div>
...
export function MapScreen() {
  ...
  const effectivePos = userPos ?? DEFAULT_CITY;
  ...
  const startGps = useCallback(() => {
    ...
    watchId.current = navigator.geolocation.watchPosition(
      ...
      (err) => {
        ...
        setGpsError(
          (MSG[err.code] ?? "Erreur GPS.") +
            ` Carte centrée sur ${DEFAULT_CITY.name}.`,
        );
        setGpsLoading(false);
      },
  ...
  const recenter = useCallback(() => {
    const target = userPos ?? DEFAULT_CITY;
    mapRef.current?.flyTo({
  ...
  const enrichWithDistance = useCallback(
    (items, getLat, getLng) =>
      items
        .map((item) => {
          const d = Math.round(
            distanceMeters(effectivePos, {
              latitude: getLat(item),
              longitude: getLng(item),
            }),
          );
          return {
            ...item,
            computedDistance: d,
            computedWalkTime: walkMinutes(d),
          };
        })
  ...
  const nearbyWater = useMemo(
    ...
    [waterStations, enrichWithDistance],
  );
    ...
    () =>
      [
        ...nearbyCool.map((s) => ({
          ...
          kind: "cool",
          data: s,
          computedDistance: s.computedDistance,
          computedWalkTime: s.computedWalkTime,
        })),
        ...nearbyWater.map((s) => ({
          id: `water-${s.id}`,
          kind: "water",
  ...
  return (
    <div className="min-h-full bg-slate-50 overflow-x-hidden">
      ...
      <div className="bg-white border-b border-slate-200 p-4 sm:p-4 shadow-sm">
        ...
        <div className="text-slate-500 text-sm mt-0.5 space-y-1">
          ...
          <p className="text-xs">
            {userPos
              ? "Position GPS active"
              : `Carte centrée sur ${DEFAULT_CITY.label}`}
          </p>
        </div>
      </div>
      ...
      <div className="flex gap-2 px-4 py-3 bg-white border-b border-slate-100 flex-wrap">
        {[
          {
            label: "Zones fraîches",
            active: showCool,
            toggle: () => setShowCool((v) => !v),
            on: "bg-sky-100 border-sky-300 text-sky-700",
            icon: <Trees size={13} />,
          },
          {
            label: "Zones chaudes",
          ...
          },
          {
            label: "Fontaines",
            active: showWater,
            toggle: () => setShowWater((v) => !v),
            on: "bg-cyan-100 border-cyan-300 text-cyan-700",
      ...
      <div style={{ height: "min(54vh, 520px)", position: "relative" }}>
            ...
            <div>
              ...
              <p className="text-slate-700 text-sm">
                ...
                <span className="text-slate-400 text-xs">
                  Assurez-vous que FastAPI tourne sur le port 8000.
                </span>
              </p>
            </div>
          </div>
        ...
        <Map
          ref={mapRef}
          initialViewState={{ ...DEFAULT_CITY, zoom: 13.5 }}
            ...
            {activeNavigation && userPos ? (
              <div
                style={{
                  ...
                  borderRadius: "50%",
                  background: "#dbeafe",
                  boxShadow: "0 0 0 4px #93c5fd77, 0 4px 12px rgba(2,6,23,.2)",
                }}
              >
          ...
          {showHeat &&
            heatZones.map((z) => (
              <HeatMarker
                ...
                zone={z}
                selected={
                  selected?.kind === "heat" && selected.data.id === z.id
                }
                onClick={(v) => setSelected({ kind: "heat", data: v })}
              />
            ))}
          {showWater &&
            waterStations.map((w) => (
              <WaterMarker
                ...
                station={w}
                selected={
                  selected?.kind === "water" && selected.data.id === w.id
                }
                onClick={(v) => setSelected({ kind: "water", data: v })}
              />
            ))}
          {showCool &&
            coolSpots.map((s) => (
              <CoolMarker
                ...
                spot={s}
                selected={
                  selected?.kind === "cool" && selected.data.id === s.id
                }
                onClick={(v) => setSelected({ kind: "cool", data: v })}
              />
            ))}
        </Map>
      </div>
      ...
      <div className="px-4 py-2 bg-white border-b border-slate-100 flex flex-wrap gap-3 text-xs text-slate-500">
        {[
          {
            color: COOL_ZONE_COLORS.water.high,
            label: "Eau",
            shape: "rounded-full",
          },
          {
            color: COOL_ZONE_COLORS.park,
            label: "Boisé",
            shape: "rounded-full",
          },
          {
            color: COOL_ZONE_COLORS.green,
            label: "Arboré",
            shape: "rounded-full",
          },
          {
            color: COOL_ZONE_COLORS.water.base,
            label: "Fontaines",
            shape: "rounded-full",
          },
          ...
          <span key={label} className="flex items-center gap-1">
            <span
              className={`w-3 h-3 inline-block ${shape}`}
      ...
      <div className="p-4 pb-8">
        ...
        {selected?.kind === "heat" &&
          (() => {
            ...
            return (
              <Card
                className="p-4 sm:p-5 shadow-xl border-2 mb-4"
                style={{ borderColor: c }}
              >
                ...
                >
                  <AlertTriangle
                    size={16}
                    style={{ color: c, flexShrink: 0, marginTop: 2 }}
                  />
                  <p className="text-slate-700">
                    Zone à forte accumulation de chaleur — limitez votre
                    exposition.
                  </p>
                </div>
              </Card>
          ...
          <>
            ...
            >
              <div className="flex items-start gap-4 mb-3">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-slate-900 mb-1">
                    {selected.data.nom_fontaine}
                  </h3>
                  <p className="text-sm text-slate-500">
                    {selected.data.adresse}
                  </p>
                </div>
                <button
                  onClick={() => setSelected(null)}
          ...
          <>
            ...
            <div className="space-y-2 mb-6">
              {nearbyPlaces.slice(0, 5).map((place) => {
                ...
                const iconClr = isCool
                  ? coolColor(item.coolnessScore, item.type)
                  : "#06b6d4";
                return (
                  <Card
                    key={place.id}
                    className="p-3 cursor-pointer hover:shadow-md transition-shadow border-slate-200"
                    onClick={() =>
                      setSelected({ kind: place.kind, data: item })
                    }
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className="w-10 h-10 rounded-xl flex items-center justify-center text-white flex-shrink-0"
                        style={{ background: iconClr }}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500">
                          {place.computedDistance} m · {place.computedWalkTime}{" "}
                          min
                        </p>
                      </div>
                      {isCool ? (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-500">
                          {z.computedDistance} m · Classe {z.sourceClass}
                        </p>
                      </div>
                      <span
