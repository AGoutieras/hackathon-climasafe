  ...
  return [
    {
      ...
      tips: [
        ...
        "Emportez toujours une bouteille avec vous",
      ],
    },
    {
      id: 2,
...
export function TipsScreen() {
  ...
  useEffect(() => {
    ...
    api
      .getRisks(position.latitude, position.longitude, DEFAULT_CITY.key)
      .then((res) => {
        if (!cancelled) setRiskData(res ?? null);
      })
      .catch((err) => {
  ...
  return (
    ...
    >
      ...
      <div className="pt-4 pb-6">
        <h1 className="text-2xl sm:text-4xl mb-2 text-slate-900">
          Nos conseils 💡
        </h1>
        <p className="text-slate-600 text-sm sm:text-lg">
          Protégez-vous de la chaleur ({tempLabel})
        </p>
      </div>

      {/* Warning card */}
      <Card
        className={`p-4 sm:p-5 mb-6 bg-gradient-to-r ${ui.warningGradient} text-white border-0 shadow-lg`}
      >
        ...
        {tips.map(({ id, category, icon: Icon, iconBg, tips: items }) => (
          <Card key={id} className="p-4 sm:p-5 shadow-sm border-slate-200">
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`w-10 h-10 sm:w-12 sm:h-12 ${iconBg} rounded-xl flex items-center justify-center text-white`}
              >
                <Icon size={24} />
              </div>
      ...
      <Card className="p-5 mb-6 bg-blue-50 border-blue-200 shadow-sm">
        ...
        <div className="space-y-2 text-sm">
          {[
            ...
            {
              href: "tel:0800066666",
              label: "0800 06 66 66 - Canicule Info Service",
            },
          ].map(({ href, label }) => (
            <a
              key={href}
